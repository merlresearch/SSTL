...
load('spiralSynthetic10Data100N30m500_results.mat')
for ff = 1:10

    filename = strcat('Datasets/spiralSynthetic10Data100N30m500v', num2str(ff));
    ...
    options=make_options('gamma_I',1,'gamma_A',1e-5,'NN',6,'KernelParam',3.0);
    options.Verbose=1;
    options.UseBias=0; %This is bias in SVM.
    options.UseHinge=1; %Hinge loss for binary/mullticlass classification
    ...
    for ii =  1:length(datasets)
        y =  datasets{ii}.y;
        ytest = datasets{ii}.testy;
        y(y==0) = -1;
        ytest(ytest==0) = -1;
        datasets{ii}.y = y;
        datasets{ii}.testy = ytest;
    ...
    numb_train_task = 25; % Total number of training datasets
    for ii = 1:length(datasets)
        if ii<=numb_train_task
            ...
            YComplete(1:length(X_train)) = datasets{ii}.y;
            Y = [Y; YComplete ];
            Y_train =  datasets{ii}.y;
            Y_test = datasets{ii}.testy;
            YOrig = [YOrig; Y_train;Y_test ];
            Lap{ii} = laplacian(options,XComplete);
            numb_examples(ii,1) = length(XComplete);
        else

            X_test = datasets{ii}.testx;
            X = [X; X_test ];
            YComplete = zeros(length(X_test),1);
            Y = [Y; YComplete ];
            Y_test = datasets{ii}.testy;
            YOrig = [YOrig;Y_test ];
            Lap{ii} = laplacian(options,X_test);
            numb_examples(ii,1) = length(X_test);
        end
    end
    ...
    for rr = 1:20
        ...
        bw_kde1 = bw1_est_all(rr,ff); %1.6238; % bandwidth of the kernel on X
        bw_kde2 = bw2_est_all(rr,ff); %14.3845  ; % bandwidth of the kernel on X for estimating P_X
        bw_kde3 = bw3_est_all(rr,ff); %0.7848; % bandwidth of the kernel on P_X
        ...
        for ii=1:length(datasets)
            ...
            if ii<=numb_train_task
                ...
                total_len = length(xitrain) + length(xitest);
        ...
        test_error(rr,ff) = 100*sum(out(numb_training_examples+1:end) ~= YOrig(numb_training_examples+1:end))/length(YOrig(numb_training_examples+1:end));
        fprintf('Error rate=%.1f\n\n',test_error(rr,ff));
    end
...
% Save the Results
result_filename = strcat('spiralSynthetic10Data100N30m500v','_sslresults');
